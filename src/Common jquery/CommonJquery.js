...
const check_vaild_save = (class_name) => {
  ...
  $("#" + class_name + " .new_password").each(function () {
    new_passwordlElement = $(this).attr("class").includes("new_password");
...
const handleEmailChange = (e) => {
  ...
  // Check if the input contains any characters other than letters, digits, '@', and '.'
...
const computeTodayDateCustom = (input_date, adddays = 0) => {
  ...
  const maxDate = new Date(
    today.getFullYear(),
    today.getMonth(),
    today.getDate() + adddays + 1
  );
  console.log(input_date, adddays, maxDate.toISOString().split("T")[0]);
  return maxDate.toISOString().split("T")[0]; // Format as YYYY-MM-DD
};
...
const formatTimeintotwodigit = (timeString) => {
  ...
  //   hourCycle: "h24", // Specify hourCycle as "h24" for 24-hour format
  //   hour: "2-digit",
  //   minute: "2-digit",
  // });
};
